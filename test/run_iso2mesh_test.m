...
if (ismember('prim', tests))
    ...
    test_iso2mesh('meshabox elem', @savejson, round(sum(elemvolume(no,el)),2), '[1]');
    [no,fc]=meshabox([0 10 5], [10 20 30], 1);
    test_iso2mesh('meshabox offset', @savejson, round(sum(elemvolume(no,fc)),2), '[1200]');

    [no,fc,el]=meshunitsphere(0.1,100);
    test_iso2mesh('meshunitsphere face', @savejson, round(sum(elemvolume(no,fc)),4), '[12.5119]');
    test_iso2mesh('meshunitsphere elem', @savejson, round(sum(elemvolume(no,el)),4), '[4.1547]');

    [no,fc,el]=meshasphere([1,1,1], 2, 0.2,100);
    test_iso2mesh('meshasphere face', @savejson, round(sum(elemvolume(no,fc)),4), '[50.0476]');
    test_iso2mesh('meshasphere elem', @savejson, round(sum(elemvolume(no,el)),4), '[33.2376]');
    ...
    test_iso2mesh('meshanellip face', @savejson, round(sum(elemvolume(no,fc)),4), '[62.4487]');
    test_iso2mesh('meshanellip elem', @savejson, round(sum(elemvolume(no,el(:,1:4))),4), '[32.5608]');

    [no,fc,el]=meshacylinder([1 1 1],[2 3 4],[10,12],0.1,10);
    test_iso2mesh('meshacylinder face', @savejson, round(sum(elemvolume(no,fc)),4), '[1045.2322]');
    test_iso2mesh('meshacylinder elem', @savejson, round(sum(elemvolume(no,el(:,1:4))),4), '[1402.8993]');
...
if (ismember('core', tests))
    ...
    test_iso2mesh('v2s face', @savejson, round(sum(elemvolume(no,fc(:,1:3))),4), '[5.0082]');

    [no,el,fc]=v2m(im,0.5,0.03,10);
    test_iso2mesh('v2m face', @savejson, round(sum(elemvolume(no,fc(:,1:3))),4), '[5.0082]');
    test_iso2mesh('v2m elem', @savejson, round(sum(elemvolume(no,el(:,1:4))),4), '[0.8786]');
end
